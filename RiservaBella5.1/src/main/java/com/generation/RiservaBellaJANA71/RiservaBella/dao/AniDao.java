...
public class AniDao {
	...
	public List<Animale> leggiAdozioni(String id){
		return read(Query.ADOZIONI_ANIMALI, id+ "");
	}
	
	public boolean create(Animale a) {
		return db.update(Query.INSERT_ANIMALE, a.getNome(), a.getSesso(), a.getClasse(), a.getImg(), a.getDescr());
	}
	...
	public boolean update(Animale a) {
		return db.update(Query.UPDATE_ANIMALE, a.getNome(), a.getSesso(), a.getClasse(), a.getDescr(), a.getId() + "");
	}
	
	public boolean updateImg(String filePath, Animale a ) {
		return db.update(Query.ANIMALE_IMG, filePath, a.getId()+"");
	}
	...
	public boolean adotta(int idUser, int idAn) {
		return db.update("INSERT into ADOZIONIA (ID_USER, ID_ANIMALE) values (?, ?)", idUser + "",  idAn + "");
	}
}